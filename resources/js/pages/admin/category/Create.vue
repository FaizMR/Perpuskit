<script setup lang="ts">
import InputError from '@/components/InputError.vue';
import { Button } from '@/components/ui/button';
import {
    Card,
    CardContent,
    CardDescription,
    CardFooter,
    CardHeader,
    CardTitle,
} from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import AppLayout from '@/layouts/AppLayout.vue';
import { create, index } from '@/routes/categories';
import { BreadcrumbItem } from '@/types';
import { useForm } from '@inertiajs/vue3';
import { LoaderCircle } from 'lucide-vue-next';

const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Kategori Buku',
        href: index().url,
    },
    {
        title: 'Tambah Kategori Buku',
        href: create().url,
    },
];

const form = useForm({
    name: '',
});
const submit = () => {
    form.post(index().url, {
        onSuccess: () => {
            // Reset form fields after successful submission
            form.reset();
        },
        preserveState: false,
    });
};
const setCustomMessage = (event: Event) => {
    const input = event.target as HTMLInputElement;
    if (input.validity.valueMissing) {
        if (input.id === 'name') {
            input.setCustomValidity('Nama Kategori wajib diisi.');
        }
    } else {
        input.setCustomValidity('');
    }
};
const clearCustomMessage = (event: Event) => {
    const input = event.target as HTMLInputElement;
    input.setCustomValidity('');
};
</script>
<template>
    <!-- <Head title="Daftar Kategori" /> -->
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="rounded-x1 flex h-full flex-1 flex-col gap-4 p-4">
            <div class="mx-auto mt-2 md:mt-8 p-6">
                <form @submit.prevent="submit">
                    <Card class="mx-auto w-full max-w-sm md:max-w-6xl">
                        <CardHeader>
                            <CardTitle>Tambah Kategori Buku</CardTitle>
                            <CardDescription>Silakan isi data Kategori.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div class="grid grid-cols-1 gap-y-6 md:grid-cols-2 md:gap-x-4">
                                <div class="grid gap-2">
                                    <Label for="name">Nama Kategori</Label>
                                    <Input id="name" type="text" required :tabindex="2" autocomplete="name"
                                        v-model="form.name" placeholder="Masukkan nama kategori" ref="nameInput"
                                        @invalid="setCustomMessage" @input="clearCustomMessage" />
                                    <InputError :message="form.errors.name" />
                                </div>
                            </div>
                        </CardContent>
                        <CardFooter class="flex justify-between px-6 pt-5 pb-6">
                            <Button type="submit" class="mt-4 w-full" :tabindex="9" :disabled="form.processing">
                                <LoaderCircle v-if="form.processing" class="h-4 w-4 animate-spin" />
                                Simpan
                            </Button>
                        </CardFooter>
                    </Card>
                </form>
            </div>
        </div>
    </AppLayout>
</template>
