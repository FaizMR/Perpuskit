...
import { destroy, index, show } from '@/routes/riwayatpeminjamans';
import {
    BreadcrumbItem,
    PaginatedResponse,
    PengajuanPeminjaman,
...
const props = defineProps<{
    LoanHistory: PaginatedResponse<PengajuanPeminjaman>;
}>();
console.log(props.LoanHistory);
const pagination = computed(() => ({
    previous: props.LoanHistory.prev_page_url,
    next: props.LoanHistory.next_page_url,
}));
...
const status = ref(pageProps.value.status ?? '');
const perPage = ref(pageProps.value.perPage ?? '7');
const updateRiwayatPeminjamans = () => {
    router.get(
        '/riwayatpeminjamans',
        {
...
function toggleSort(key: string) {
    ...
    updateRiwayatPeminjamans();
}
watchDebounced(
    [searchQuery, status, searchBy, perPage],
    () => {
        updateRiwayatPeminjamans();
    },
    { debounce: 500 },
        ...
        <div class="md:mx-auto mt-10 max-w-sm md:max-w-6xl overflow-x-auto md:mt-5">
            ...
            <Card class="border-transparent mt-2">
                <CardContent>
                    <div class="flex flex-col items-stretch justify-between gap-4 sm:flex-row sm:items-center">
                        <div class="flex items-end gap-2">
                            <Input id="searchQuery" class="w-full sm:w-64" v-model="searchQuery"
                                placeholder="Cari..." />
                            <select id="searchBy" v-model="searchBy" @change="updateRiwayatPeminjamans"
                                ...
                                <Popover>
                                    ...
                                    <PopoverContent>
                                        <div class="flex flex-col">
                                            ...
                                            <Label for="perPage" class="mt-3 mb-2">
                                                Jumlah Data
                                            </Label>
                    ...
                    <DataTable :columns="columns" :data="LoanHistory.data" :links="LoanHistory.links"
                        :current_page="props.LoanHistory.current_page" :per_page="props.LoanHistory.per_page" :filters="{
                            search: searchQuery,
